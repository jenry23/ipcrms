<template>
	...
	<div class="col-md-1 mt-3">
		<button class="btn btn-sm btn-success" @click.prevent="downloadFiles">
			Download
		</button>
	</div>
		<div class="card-body">
			...
			<table class="table table-border two-table">
				...
				<tbody v-for="(functions, index) in templates.ipcr_function" :key="index">
					...
					<tr v-for="(subFunction, index1) in functions.ipcr_subfunctions" :key="index1">
						<td colspan="10">{{ subFunction.name }}</td>
					</tr>
					<tr v-for="(performance, index2) in functions.ipcr_performance" :key="index2 + performance">
						<td>{{ performance.name }}</td>
						<td style="display: none"><input type="text" v-model="performance.id" disabled/></td>
						<td><input type="number" v-model="performance.target" disabled /></td>
						<td><input type="number" v-model="performance.accomplished" disabled /></td>
						<td>
							<input type="date" v-model="performance.date_of_submission" disabled />
						</td>
						<td><input type="date" v-model="performance.date_completed" disabled /></td>
						<td>
				...
				<tfoot>
					<tr>
						<td colspan="5">
							...
							<input type="text" v-model="templates.ipcr_function.recommendation" size="70" disabled />
						</td>
...
export default {
	...
	methods: {
		downloadFiles () {
			const doc = new jsPDF();
			const contentHtml = this.$refs.faculty_download.innerHTML;
